...
class WorkerThread (threading.Thread):
    ...
    def filterBinary(self, header, patch, threshold):
        newMessage = [0] * self.patchsize * self.patchsize
        ...
        while(True):
            if self.inQueue.qsize() > 0:
                ...
                if str(message[0][0]) == "PrewittFilter":
                    outMessage = self.filterPrewitt(message[0][1], message[1],
                                                  128)
                if str(message[0][0]) == "Binarization":
                    outMessage = self.filterBinary(message[0][1], message[1],
                                                  128)
                if str(message[0][0]) == "GrayScale":
                    outMessage = self.convertGray(message[0][1], message[1])
                if str(message[0][0]) == "Gaussian":
                    outMessage = self.filterGaussian(message[0][1], message[1],128)
                # self.pLock.acquire()
...
class imGui(QMainWindow):
    def __init__(self, workQueue, processedQueue, pLock):
        ...
        self.ui.boxFunction.addItem("Binarization")
        self.ui.boxFunction.addItem("Gaussian")
